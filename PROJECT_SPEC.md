...
쇼츠 형식을 선택하면 영상 길이 60초와 도서 수 1권을 고정한다. 같은 근거 우선 파이프라인으로 주제에 가장 직접적인 한 권을 선정하고 `생활형 훅 → 책 공개 → 핵심 통찰 → 한 문장 적용·여운`의 4개 섹션을 생성한다.
...
* `content_format=shorts`는 근거가 충분한 도서 정확히 1권을 선정
...
일반 영상은 여러 책을 하나의 스토리라인으로 연결하고, 쇼츠는 한 권의 핵심 통찰을 60초 구조로 설계한다.
...
* 섹션별 예상 시간 계산
* 제목 후보 생성
* 쇼츠는 `hook → book_intro → book_perspective → conclusion` 4개 섹션과 총 60초를 강제
...
* 쇼츠는 책 제목과 저자를 `book_intro`에서 한 번 소개하고 화면 인용을 최대 1개 사용
...
* 검증 이슈 문단 선택, 불변 부분 리비전 생성과 자동 재검증 API
...
* 고위험 요약·인과관계 이슈 문단 선택과 재작성 후 자동 재검증
* 산출물 다운로드
...
도서 수
[2권] [3권] [4권]
쇼츠 · 한 권 선택 시 길이와 도서 수를 [60초] [1권]으로 고정
톤
[사색적] [지적] [실용적] [스토리텔링]
카테고리
[인문학] [철학] [심리학]
